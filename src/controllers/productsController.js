...
export const SearchProduct = async(req,res) =>{
    ...
    try {
        ...
        res.status(500).json({error: `Ocurrió un error al buscar el producto. ${error.message}`});
    }
}
...
export const UpdateProduct = async(req, res) =>{
    ...
    try{
        ...
        if(!productFound){
            res.status(500).json({error: `No se encontró el producto asociado al ID: ${productId}`});
            return
        }
    } catch (error) {
        res.status(500).json({error: `Ocurrió un error al buscar el producto. ${error.message}`});
    ...
    if(!newName && !newDescription && !newPrice && !newIsAvailable){
        res.status(400).json({error: "Al menos uno los campos requeridos debe estar completo para actualizar."})
        return
    }
